---
layout: post
title: Standards Rust Style Guide
---
...
world. I hope to get this into rustfmt in the near future, and as an alternative
in the style RFC process as well.

The intention of this style guide is to be

 * Dead simple to follow. You should not be questioning yourself "do I put this
   here? or here?"
...
SECOND DRAFT. These formatting conventions are a work in progress, and may do
...
important that you style your code so that it's very searchable in this way. The
proper format, therefore, is:
...
If the arguments don't fit nicely onto one line, split it, and put a comma after
the last parameter:

```rust
fn lots_of_args(
    a_thirty_two_bit_integer: i32, a_sixty_four_bit_float: f64, a_boolean: bool,
    a_character: char, an_awfully_long_named_thing: u8,
) -> u8 {
}
```
...
Note that `match`es are as short as possible. If it does go over the line limit,
then block indent.
...
These are pretty easy. The matchers are already indented; So, if you need to go
to the next line, keep the same indentation. As with other binops, a `|` will go
to the next line, not stay on the previous line. Make sure the `=>` is on its
own line. If you must block indent the matchee, use `{}` to create an actual
block (unless it's already a block -- see after the example).
...
impl Token {
    pub fn ty(&self) -> TokenType {
        match *self {
            ...
            t @ Token::KeywordElse | t @ Token::OpenParen
            | t @ Token::CloseParen | t @ Token::Dot | t @ Token::OpenBrace
            | t @ Token::Semicolon | t @ Token::Colon | t @ Token::SkinnyArrow
            | t @ Token::Comma | t @ Token::Equals | t @ Token::Eof
            => {
...
Any delimited expression (`unsafe { ... }`, `for _ in _ { ... }`,
`if _ { ...  }`, `match _ { ... }`, constructors, function calls) may be treated
as if they start at the indent on which the matchee starts on, and a normal
block.
...
a return type, when there are statements before the final expression, or when
you need to split it into multiple lines.

If the argument list of the closure is long enough to require splitting, use the
`||` as a block for block indenting.
...
|
    ...
    argument_six, argument_seven, argument_eight, argument_nine, argument_ten
| {
    println!("{}", argument_nine + argument_ten);
    {
        argument_one + argument_two + argument_three + argument_four
        + argument_five + argument_six + argument_seven + argument_eight
    }
}
```
...
If a continuing line is required, i.e., generics, arguments, a type, an
expression, or a statement is long enough to be split over multiple lines,
what's known as "block indenting" is used. Split it after a comma, or before an
operator; and, in an expression, if there is no block, use either `{}` or `()`
to make a block. Usually, you'd just use a `let` variable or two to make sure
...
fn really_long_generics<
    GenericOne, GenericTwo, GenericThree, GenericFour, GenericFive, GenericSix,
>(
    par_one: GenericOne, par_two: GenericTwo, par_three: GenericThree,
    param_four: GenericFour, par_five: GenericFive, par_six: GenericSix,
) -> bool where
    GenericOne: BoundOne, GenericTwo: BoundTwo, GenericThree: BoundThree + Copy,
    GenericFour: BoundFour, GenericFive: BoundFive, GenericSix: BoundSix,
{
    ...
}
```
...
DO include extraneous parentheses if it makes an arithmetic expression easier to
understand (`(x * 15) + (y * 20)` is fine)
...
`if-else` expressions, if you can fit the entirety of the `if-else` block on one
line *with* the left-hand side of the statement, one may put it all on one line:
...
Otherwise, if the test is too long to fit on the same line, then put it inside a
block, and block indent:
...
This happens if you have a struct literal as the only argument to a single
argument function. If there are any other arguments, use a multi-line function
call.
...
DO keep `struct`, `[Name]`, and `{` on the same line, even if it breaks the line
limit.
...
DO have `struct`, `[Name]`, and `(` on the same line, even if it breaks the line
limit.
...
DO format each variant accordingly as either a `struct`, `tuple struct`, or just
simply as an ident, which doesn't require special formatting.
...
For c-style enums that have expressions on their right hand side, it is possible
to go over the line limit. In that case, use block indenting without the block
(we realize it's ugly; honestly, we just can't think of a better way. Don't
break the line limit in your own code):
...
`macro_rules!` formatting acts like one would expect; if the "matcher" goes over
the line limit, block indent it, and if the "block" part goes over the line
limit, block indent it. If, of the two, you must make the choice of which to
block indent, prefer the second.
...
A string literal which has a newline may have a line break in the string itself;
in that case, block indent the string.  Remember to end each line with a `\`:
...
the first line, or the last line, for comments. Block indent the comment inside.
The final `*/` should line up in the zeroth column.
...
Put each attribute on it's own line, indented to the indentation of its item. In
the case of inner attributes (`#!`), indent it to the inner indentation (the
 ...
 * https://github.com/rust-lang/rust/pull/35614#discussion-diff-74550692
 * https://github.com/rust-lang/rust/pull/34220/files#diff-4f7de8be158a8761fb58795c75249dc9L24
 * https://github.com/rust-lang/rust/pull/34211/files#diff-68d362312a8dd3bf52387ad1b96cbf53L570
 * https://github.com/rust-lang/rust/pull/34211/files#diff-68d362312a8dd3bf52387ad1b96cbf53L622
