...
If the arguments don't fit nicely onto one line, split it after the `(`, and
after each argument, and put a comma after the last parameter:
...
to the next line, keep the same indentation. As with other binops, a `|` will
go to the next line, not stay on the previous line. Make sure the `=>` is on
its own line. If you must block indent the matchee, use `{}` to create an
actual block (unless it's already a block -- see after the example).
...
`if _ { ...  }`, `match _ { ... }`, constructors, function calls) may be
treated as if they start at the indent on which the matchee starts on, and a
normal block.
...
If the argument list of the closure is long enough to require splitting, use
