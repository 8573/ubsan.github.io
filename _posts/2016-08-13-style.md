...
As of August 2016 when this style guide was started, the standard `rustfmt`
...
In contrast, the <dfn>block indentation</dfn> used in this style (to the
...
<!-- TODO(ubsan): <c74d> I don't know that I properly understood what this
    meant: "in an expression, if there is no block, use either `{}` or `()` to
    make a block". Is it meant to apply to all expressions, not just control
    flow conditionals? -->
...
If the expression to be split contains a sub-expression delimited with
...
<!-- TODO: <c74d> How is this to be formatted?
    let x = this_is_a_variable_with_an_unusually_long_name * (this_is_another_excessively_long_variable_name + why_are_these_names_so_long) + foo_bar_baz;
-->
...
Note that, although excessive line length is conferred upon this section's
...
<!-- TODO(ubsan): <c74d> How are `where` clauses in associated function
     declarations with no function bodies to be formatted? -->
...
<!-- TODO(ubsan): <c74d> Like what extra spaces? -->
...
<!-- TODO(c74d): Process these two examples. -->
...
#### §3.1. Multi-line function headers
...
<!-- TODO: Should this be moved to §2? -->

Prefer `where` to `T: Trait`, except for `?Sized`, of course, or where
supporting an older compiler is necessary.
<!-- TODO: Should this be moved to §2.3.1, if that is not moved here? -->

### §4. Function Body

<!-- TODO(ubsan): <c74d> I made up much of this, so make sure you agree. -->
...
[Figure 4.1](#fig-4.1) gives a general example of this style.
...
<!-- TODO(ubsan): <c74d> I don't know what a "matchee" is, and some parts of
     this seemed to be vague or in conflict with the code example, so I'm
     trying to reconstruct your intentions here, and I may well have gotten
     them wrong. -->

In this style, a `match` expression is to be written with its block of match
...
<!-- <c74d> The term "arm body" was made up by eddyb when I asked what to call
     the thing after the `=>`, with "arm pattern" to go along with it.
     TODO(c74d): Credit eddyb for the terms in a footnote. -->
...
If a match arm would exceed the line length limit, and block-indentation of
